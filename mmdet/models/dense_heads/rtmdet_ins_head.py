...
class RTMDetInsSepBNHead(RTMDetInsHead):
    ...
    def forward(self, feats: Tuple[Tensor, ...]) -> tuple:
        ...
        # # ===========For ncnn 
        # res = []
        # for i,j in enumerate(cls_scores):
        #     b,c,h,w = j.shape
        #     j.permute(0, 2, 3, 1).reshape(b, -1, c).softmax(-1)
        #     cls_scores[i] = j.reshape(b, h, w, c).permute(0, 3, 1, 2).contiguous()
        # for feats in zip(*(tuple(cls_scores), tuple(bbox_preds), tuple(kernel_preds))):
        #     res.append(torch.cat(feats, 1).permute(0, 2, 3, 1))

        # res.append(mask_feat)
        # return tuple(res)