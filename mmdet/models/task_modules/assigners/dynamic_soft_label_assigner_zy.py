...
class ZY_DynamicSoftLabelAssigner(BaseAssigner):
    ...
    def __init__(
        ...
        soft_cls_weight: int = 1,
        soft_center_weight: int = 1
    ) -> None:
        ...
        self.soft_center_weight = soft_center_weight
        self.cls_cost = TASK_UTILS.build(dict(type='DynamicFocalLossCost', weight=1.0))
    ...
    def assign(self,
        ...
        num_valid = valid_decoded_bbox.size(0) # 有多少预测
        ...
        """binary_cross_entropy_with_logits for cls.
            soft_cls_cost
            gt_onehot_label = (
            ...
            valid_decoded_bbox, gt_bboxes, valid_pred_scores, soft_label
            soft_cls_cost = sigmoid_focal_loss(pred_scores, valid_pred_scores).sum(dim=-1)
            soft_cls_cost = GFocalLoss(pred_scores, gt_labels)
        """
        soft_cls_cost = self.cls_cost(pred_instances=valid_pred_scores,
                                    gt_instances=gt_instances,)

        # weight
        cost_matrix = soft_cls_cost * self.soft_cls_weight + iou_cost * self.iou_weight + soft_center_prior * self.soft_center_weight
