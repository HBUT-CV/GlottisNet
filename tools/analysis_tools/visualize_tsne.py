...
from mmdet.registry import DATA_SAMPLERS, DATASETS

#! python tools/analysis_tools/visualize_tsne.py ${CONFIG_FILE} --checkpoint ${CKPT_PATH} --work-dir ${WORK_DIR} [optional arguments]
#  python tools/analysis_tools/visualize_tsne.py /home/ren2/data3/ZhangYang/mmdetection_v3x_glottis/configs/rtmdet/zy_rtmdet_ins_tiny_8xb32-300e_coco_all.py --checkpoint /home/ren2/data3/ZhangYang/mmdetection_v3x_glottis/work_dirs_ins/zy_run_noPretrans/zy_rtmdet-ins_tiny_detcov0_MyBlock1_OnlyNeck_k=3_FLassign133Topk9_320/best_coco/bbox_mAP_epoch_308.pth  --work-dir /home/ren2/data3/ZhangYang/mmdetection_v3x_glottis/work_dirs_ins/zy_run_noPretrans/zy_rtmdet-ins_tiny_detcov0_MyBlock1_OnlyNeck_k=3_FLassign133Topk9_320/vis_tsne
...
def main():
    ...
    #@ add for mmdet
    extract_dataloader_cfg = cfg.get('test_dataloader')
    ...
    for i in range(len(dataset)):
        ...
        #@ add for mmdet
        gts = dataset.get_data_info(i)['instances']
        if np.zeros(len(gts)) < args.max_num_class:
    ...
    for _, data in enumerate(tsne_dataloader):
        with torch.no_grad():
            ...
            #@ add for mmdet
            batch_features = model.extract_feat(batch_inputs)
            ...
            #@ add for mmdet
            # batch_labels = torch.cat([i.gt_instances for i in batch_data_samples])
            batch_labels = torch.LongTensor(1)
            # batch_labels = dataset.get_data_info(batch_data_samples[0].img_id)['instances']
    ...
    for key, val in results.items():
        ...
        #@ add for mmdet
        result = tsne_model.fit_transform(val.reshape(-1,1))
